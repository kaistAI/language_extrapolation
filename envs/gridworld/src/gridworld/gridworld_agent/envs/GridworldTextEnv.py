...
DEFAULT_FILE_PATH = join(BASE_DIR, '../../data/inference.jsonl')

class GridworldTextEnv():
    ...
    def __init__(
        ...
        show_attrs
        """
        super(GridworldTextEnv, self).__init__()
        self.kwargs = kwargs
...
class GridBoard:
    ...
    #     displ_board = np.zeros((num_pieces, self.xsize, self.ysize), dtype=np.uint8)
    #     layer = 0
    #     for name, piece in self.components.items():
    #         pos = (layer,) + piece.pos
    #         displ_board[pos] = 1
...
class Gridworld:
    ...
    def checkMove(self, old_pos, action):
        ...
        # if new_pos in [item.pos for item in self.board.findPiecebyName('Wall')]:
        #     return old_pos #block move, player can't move to wall
        # elif new_pos[0] > (self.board.xsize-1) or new_pos[1] > (self.board.ysize-1):    #if outside bounds of board
        #     return old_pos
    ...
    def makeMove(self, action):
        ...
        #actions in {u,d,l,r}
        player = self.board.findPiecebyName('Player')[0]
        old_pos = player.pos
        self.board.movePiece(player.id, self.checkMove(player.pos, action))
        if self.board.findPiecebyName('Player')[0].pos in [item.pos for item in self.board.findPiecebyName('Pit')] or self.board.findPiecebyName('Player')[0].pos in [item.pos for item in self.board.findPiecebyName('Wall')] or self.board.findPiecebyName('Player')[0].pos[0] > (self.xstart + self.xsize-1) or self.board.findPiecebyName('Player')[0].pos[1] > (self.ystart + self.ysize-1) or self.board.findPiecebyName('Player')[0].pos[0] < self.xstart or self.board.findPiecebyName('Player')[0].pos[1] < self.ystart:
            ...
            invalid = False
        return {'invalid': invalid, 'reward': self.getReward(self.board.findPiecebyName('Player')[0].pos), 'done': self.Done()}
        ...
        while True:
            ...
            for x in range(self.xstart, self.xstart+self.xsize):
                for y in range(self.ystart, self.ystart + self.ysize):
                    ...
                    #     print(f"State: {x,y}, is piece: {self.board.isPiece((x, y))}, piece: {self.board.findPiecebyPOS((x,y)).name}")
                    # else:
                    #     print(f"State: {x,y}, is piece: {self.board.isPiece((x, y))}")
                    if not (self.board.isPiece((x, y)) and self.board.findPiecebyPOS((x,y)).name in ['Pit', 'Goal', 'Wall']):
                        for action in self.getActions():
                            # print(f"State: {x,y}, action: {action} to {self.checkMove((x,y), action)}")
                            update_qvalue = self.getOptimalQValue((x,y), action)
                            # print(f"Q-value for state {x,y} action {action} is {update_qvalue}")
                            if update_qvalue != self.q_table[(x,y)][action]:
                                # print(f"Updating q-value for state {x,y} action {action} from {self.q_table[(x,y)][action]} to {update_qvalue}")
                    ...
                    if not (self.board.isPiece((x, y)) and self.board.findPiecebyPOS((x,y)).name in ['Pit', 'Goal', 'Wall']):
                        for action in self.getActions():
                            ...
                            if update_length < self.l_table[(x,y)][action]:
                                # print(f"Updating q-value for state {x,y} action {action} from {self.l_table[(x,y)][action]} to {update_qvalue}")
