        ...
        if optionize is not None:
            ...
            for batch, opt in enumerate(optionize):
                for tgt_idx, items in opt.items():
                    if tgt_idx-1 >= logits.shape[1] or tgt_idx-1 < 0:
                        continue
                    else:
                        keys = list(items.keys())
                        entropy_list.append(torch.index_select(logits[batch, tgt_idx-1], 0, torch.tensor(keys).to(logits.device)))
                        
            entropy_list = torch.stack(entropy_list).to(logits.device)
            entropy_list = entropy_list.float()
            entropy_loss = Categorical(logits = entropy_list).entropy().mean()
