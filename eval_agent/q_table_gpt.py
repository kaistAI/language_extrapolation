...
openai.api_base="FILL_IN_PLZ"
openai.api_key = "FILL_IN_PLZ"
    ...
    for x_index in range(x_size):
        for y_index in range(y_size):
            ...
            for curr_item in completion["choices"][0]["logprobs"]["content"][0]["top_logprobs"]:
                ...
                if token and token in action_to_prob.keys() and action_to_prob[token] == float("-inf"):
                    evaluated_q_table[f"{x_index},{y_index}"][token] = math.exp(curr_item.get("logprob"))
                    action_to_prob.pop(token)
            
            for act, _ in action_to_prob.items():
                evaluated_q_table[f"{x_index},{y_index}"][act] = not_in_top_k_logprob
                
    return {
        'optimal': optimal_q_table,
...
def main(args: argparse.Namespace):
    ...
    with logging_redirect_tqdm():
        ...
        for i, task in enumerate(all_tasks):
            ...
            state = interactive_loop_by_gpt(
                task, env_config, args.model_path
            )
