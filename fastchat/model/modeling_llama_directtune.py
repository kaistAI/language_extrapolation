...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if optionize is not None:
            for batch_idx, opt in enumerate(optionize):
                for tgt_idx, items in opt.items():
                    ...
                    for token, q_value in items.items():
                        if q_value < min_value:
                            min_value = q_value
                            min_key = token
                            if min_key is not None and tgt_idx < self.config.max_position_embeddings:
                                if torch.cuda.current_device() == 0:
                                    print(f"original[{batch_idx}][{tgt_idx}]: {labels[batch_idx][tgt_idx]} -> optionized: {min_key}")
                                labels[batch_idx][tgt_idx] = min_key                                
                            else:
                                print(f"[{batch_idx}][{tgt_idx}]: [{min_key}] has a WRONG position or WRONG value -> SKIP!!!")
